...
public class ButtonListener implements Listener {
    ...
    public ButtonListener() {
        Bukkit.getPluginManager().registerEvents(this, LandCore.getInstance());
    }
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                if (event.isCancelled()) {
                    Bukkit.getScheduler().runTaskLater(LandCore.getInstance(), player::updateInventory, 1L);
                }
            } else {
                if ((event.getClick() == ClickType.SHIFT_LEFT || event.getClick() == ClickType.SHIFT_RIGHT)) {
                    event.setCancelled(true);
    ...
    public void onInventoryClose(InventoryCloseEvent event) {
        ...
        player.setMetadata("scanglitch", new FixedMetadataValue(LandCore.getInstance(), true));
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (player.hasMetadata("scanglitch")) {
            player.removeMetadata("scanglitch", LandCore.getInstance());
